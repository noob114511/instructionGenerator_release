...
![image-20241028162427231](docs/3_5.png)

##### 2.4 UNPREDICTABLE、NOP与UNDEFINED指令

指令集中存在**UNPREDICTABLE、NOP、UNDEFINED**指令，其中

1. **NOP**指令会被处理器解码，执行效果为机器周期内不做任何操作，**需要录入**合法指令集。
2. **UNPREDICTABLE**指令可能被处理器解码，也可能不会，不在我们测试范围内，**需要录入**合法指令集。
3. **UNDEFINED**指令，**不需要录入合法指令集**。

## Ⅳ ARM指令集生成说明

![image-20241028162750284](docs/4_1.png)

ARM中，32bit的指令集是以一下比特开头的：

- 0b11101
- 0b11110
- 0b11111

因此使用**自定义生成模式**对非法指令生成。

提出一种解决方案

1. 指定32位非法指令集自定义规则：

   - 11101XXXXXXXXXXXXXXXXXXXXXXX
   - 11110XXXXXXXXXXXXXXXXXXXXXXXXXXX
   - 11111XXXXXXXXXXXXXXXXXXXXXXXXXXX
